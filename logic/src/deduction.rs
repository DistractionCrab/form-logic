...
impl Deduction {
	...
	fn apply_work_inner<K: KnowledgeBase>(&self, k: ResultBase<K>) -> ResultBase<K> {		
		match self {
			...
			Deduction::Let(c, f) => self.alias(k, c, f.clone()),
	...
	pub fn alias<K: KnowledgeBase>(&self, k: ResultBase<K>, c: &ConstName, f1: Form) 
	-> ResultBase<K> {
		if !k.has_const(&c) {
			k.result_const(c.clone()).result_form(Formula::Eq(Formula::Const(c.clone()).ptr(), f1))
		} else {
